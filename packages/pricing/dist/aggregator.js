...
class PriceAggregator {
    coefficients;
    searchRadiusM;
    constructor(coefficients = coefficients_js_1.BERLIN_ALEX_COEFFICIENTS, searchRadiusM = 500) {
    ...
    async estimate(input) {
        ...
        const proximityImpact = basePriceSqm * typeFactor * levelsFactor * (proximityFactor - 1);
        factors.push({
            name: 'proximity',
        ...
        const finalPriceSqm = Math.round(basePriceSqm * typeFactor * levelsFactor * proximityFactor);
        const priceTotal = input.areaSqm !== null
            ? Math.round(finalPriceSqm * input.areaSqm)
            : null;
        // 7. Calculate confidence score
        const confidence = this.calculateConfidence(method, nearbyListings.length, input.areaSqm !== null);
        return {
     ...
     */
    async getNearbyListings(centroid) {
        const [lng, lat] = centroid;
        try {
            const listings = await db_1.prisma.$queryRaw `
        SELECT 
          CASE 
     ...
     */
    getProximityFactor(centroid) {
        const [lng, lat] = centroid;
        const [centerLng, centerLat] = this.coefficients.proximity.centerPoint;
        // Simple distance calculation (Haversine would be more accurate)
        // At Berlin latitude: 1° lng ≈ 85km, 1° lat ≈ 111km
        const kmFromCenter = Math.sqrt(Math.pow((lng - centerLng) * 85, 2) +
            Math.pow((lat - centerLat) * 111, 2));
        const distanceFactor = 1 + kmFromCenter * this.coefficients.proximity.perKmFromCenter;
     ...
     */
    calculateConfidence(method, listingsCount, hasArea) {
